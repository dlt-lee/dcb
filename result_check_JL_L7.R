result_check_JL_L7<-function(num) {
  library(sjmisc)
  data_result=dcb[which(dcb$n==num),]
  data_pre=as.data.frame(read.csv(file = "dcb_data_JL_L7.csv", header = FALSE))
  data_pre=data_pre[which(data_pre$V1==num),]
  m_record_CR=as.matrix(read.csv(file = "dcb_data_CR_JL_L7.csv", header = FALSE))[-1,]
  ...
  write.csv(m_r_ab_delta, file = "dcb_data_CR_JL_L7.csv",row.names = FALSE)
}

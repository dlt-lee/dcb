dcb_L17<-function(num,number_of_core) {
  #source("element.R")
  library(xgboost)
  #read data
  f_r_ab_org<-as.matrix(read.csv(file = "dcb_data_l16.csv", header = TRUE))
  ...
  sum_l17_dcb<-c(sort(c(round(testPredictions.a1),
  ...
  sum_l17_dcb<-c(max(dcb$n)+1,sum_l17_dcb)
  # print(c('L17:',sum_l17_dcb))
  write.table(t(as.matrix(sum_l17_dcb,nrow(1))),file = "dcb_data_l17.csv",append = TRUE,col.names = FALSE,row.names = FALSE,quote=TRUE, sep=",")

  # return(sum_l17_dcb)
  return(c('L17:',sum_l17_dcb))
}

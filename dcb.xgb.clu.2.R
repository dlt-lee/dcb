dcb.xgb.clu.2<-function(trains,d_o) {
  ...
  if (threads <= 8) {
    ...
    bst.a6<-xgboost(data = trains.T[,c(55,62)],label = trains$res.a6,nrounds = 300,verbose=0,params = list(tree_method = t_m))
    bst.b1<-xgboost(data = trains.T[,c(56,63)],label = trains$res.b1,nrounds = 300,verbose=0,params = list(tree_method = t_m))
    ...
    bst.a6<-xgboost(data = trains.T[,c(55,62)],label = trains$res.a6,nrounds = 300,verbose=0,params = list(tree_method = t_m,nthread=number_of_core))
    bst.b1<-xgboost(data = trains.T[,c(56,63)],label = trains$res.b1,nrounds = 300,verbose=0,params = list(tree_method = t_m,nthread=number_of_core))

dcb.xgb.row.1<-function(trains,d_o) {
  library(xgboost)
  # threads=detectCores()
  ...
  if (threads <= 8){
    ...
    bst.a6<-xgboost(data = trains.T,label = trains$res.a6,nrounds = 300,verbose=0,params = list(tree_method = 'hist'))
    ...
    bst.a6<-xgboost(data = trains.T,label = trains$res.a6,nrounds = 300,verbose=0,params = list(tree_method = 'hist',nthread=number_of_core))

dcb_L15<-function(num,number_of_core) {
  #source("element.R")
  library(xgboost)
  #read data
  m_r_ab_org<-as.matrix(read.csv(file = "dcb_data_l14.csv", header = FALSE))[-1,]
  ...
  sum_l15_dcb<-c(sort(c(round(testPredictions.a1),
  ...
  sum_l15_dcb<-c(max(dcb$n)+1,sum_l15_dcb)
  # print(c('L15:',sum_l15_dcb))
  write.table(t(as.matrix(sum_l15_dcb,nrow(1))),file = "dcb_data_l15.csv",append = TRUE,col.names = FALSE,row.names = FALSE,quote=TRUE, sep=",")
 
  # return(sum_l15_dcb)
  return(c('L15:',sum_l15_dcb))
}

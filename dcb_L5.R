dcb_L5<-function(num,number_of_core) {
  ...
  library(xgboost)
  #read data
  f_r_ab_org<-as.matrix(read.csv(file = "dcb_data_l4.csv", header = TRUE))
  ...
  write.table(t(as.matrix(sum_l5_dcb,nrow(1))),file = "dcb_data_l5.csv",append = TRUE,col.names = FALSE,row.names = FALSE,quote=TRUE, sep=",")
  
  # return(sum_l5_dcb)
  return(c('L5:',sum_l5_dcb))
}

dcb_L12<-function(num,number_of_core) {
  #source("element.R")
  library(xgboost)
  #read data
  m_record_l12<-as.matrix(read.csv(file = "dcb_data_l12.csv", header = FALSE))[-1,]
  m_r_ab_org<-as.matrix(read.csv(file = "dcb_data_l11.csv", header = FALSE))[-1,]
  # filter l5_data
  ...
  sum_l12_dcb<-c(sort(c(round(testPredictions.a1),
  ...
  sum_l12_dcb<-c(max(dcb$n)+1,sum_l12_dcb)
  # print(c('L12:',sum_l12_dcb))
  m_r_ab_delta<-rbind(m_record_l12,sum_l12_dcb)
  write.csv(m_r_ab_delta, file = "dcb_data_l12.csv",row.names = FALSE)
  
  # return(sum_l12_dcb)
  return(c('L12:',sum_l12_dcb))
}

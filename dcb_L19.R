dcb_L19<-function(num,number_of_core) {
  #source("element.R")
  library(xgboost)
  #read data
  m_record_l19<-as.matrix(read.csv(file = "dcb_data_l19.csv", header = FALSE))[-1,]
  m_r_ab_org<-as.matrix(read.csv(file = "dcb_data_l18.csv", header = FALSE))[-1,]
  # filter l5_data
  ...
  sum_l19_dcb<-c(sort(c(round(testPredictions.a1),

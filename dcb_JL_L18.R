dcb_JL_L18<-function(num,number_of_core) {
  ...
  f_record_JL_L18<-as.data.frame(read.csv(file = "dcb_data_JL_L18.csv", header = FALSE))[-1,]
  ...
  f_record_l19<-as.data.frame(read.csv(file = "dcb_data_l19.csv", header = FALSE))[-1,]
  ...
  m_record_JL_L18<-as.matrix(f_record_JL_L18[f_record_JL_L18$V1<=(num+1),])[,-1]
  ...
  f_record_JL_L18$V1= as.integer(f_record_JL_L18$V1)
  ...
  m_record_l19<-as.matrix(f_record_l19[f_record_l19$V1<=(num+1),])[,-1]
  ...
  m_record_JL<-as.matrix(read.csv(file = "dcb_data_JL_L18.csv", header = FALSE))[-1,]
  m_r_ab_delta<-rbind(m_record_JL,sum_JL_dcb)
  write.csv(m_r_ab_delta, file = "dcb_data_JL_L18.csv",row.names = FALSE)
